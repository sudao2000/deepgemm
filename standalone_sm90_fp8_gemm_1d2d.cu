// standalone_sm90_fp8_gemm_1d2d.cu
//
// Self-contained SM90 (Hopper) FP8 (e4m3) GEMM with 1D2D scaling factors, based on
// deep_gemm/include/deep_gemm/impls/sm90_fp8_gemm_1d2d.cuh.
//
// Supported case (only): D[M,N](bf16) = A[M,K](fp8 e4m3, K-major) @ B[N,K]^T(fp8, K-major)
// with per-(row, 128-K-block) scales SFA[ceil(K/128), M] (1D, via TMA) and
// per-(128-N-group, 128-K-block) scales SFB[ceil(N/128)][ceil(K/128)] (2D, via plain loads).
//
// Usage: ./standalone_sm90_fp8_gemm_1d2d <m> <n> <k>
// Compile: bash compile_standalone_fp8_1d2d.sh
//
// Requirements: n % 8 == 0 and k % 16 == 0 (TMA stride constraints); m arbitrary.
...
#define DG_TRAP_ONLY_DEVICE_ASSERT(cond) \
    do { if (not (cond)) asm("trap;"); } while (0)
#endif
...
struct Scheduler {
    ...
    CUTLASS_DEVICE uint32_t get_global_idx(const uint32_t shape_dim, const uint32_t block_size,
        ...
        static_assert(kGemmType == GemmType::Normal, "This standalone scheduler only supports GemmType::Normal");
        (void)shape_dim; (void)kWithGroupOffset;
...
// ---------------------------------------------------------------------------
// Kernel (adapted from deep_gemm/include/deep_gemm/impls/sm90_fp8_gemm_1d2d.cuh)
// ---------------------------------------------------------------------------
namespace deep_gemm {
