...
def enumerate_normal(dtype: torch.dtype) -> Generator:
    ...
    m_fwd, n_fwd, k_fwd = 128, 512, 512
    m_bwd, n_bwd, k_bwd = 128, 512, 512
    ...
    for kernel_type in get_kernel_types(dtype):
        for quant_config in quant_config_list:
            ...
            # Forward
            yield kernel_type, quant_config, m_fwd, n_fwd, k_fwd, MajorTypeAB.KMajor, MajorTypeAB.KMajor, False, torch.bfloat16
            ...
            yield kernel_type,          quant_config, m_bwd, k_bwd, n_bwd, MajorTypeAB.KMajor, override_major, False, torch.bfloat16     # Dgrad
...
def enumerate_m_grouped_contiguous(dtype: torch.dtype) -> Generator:
    quant_config_list = QuantConfig.get_list_from_dtype(dtype)
    # Single small case for fast execution
    num_groups, expected_m_per_group = 4, 256
    n, k = 512, 512
    for kernel_type in get_kernel_types(dtype):
        for quant_config in quant_config_list:
            if len(quant_config_list) > 1:
...
def enumerate_k_grouped_contiguous(dtype: torch.dtype):
    # Single small case for fast execution
    if dtype == torch.bfloat16:
        k_alignment_options = [(128, 128)] if get_arch_major() == 9 else [(128, 128)]
    else:
        k_alignment_options = [(128, 128)] if get_arch_major() == 9 else [(128, 128)]
    ...
    num_groups, m, n, expected_k_per_group = 4, 256, 512, 128
...
def enumerate_k_grouped_contiguous_test_variants(real_ks_cpu: List[int], k_alignment: int,
    ...
    # Single variant for fast execution
    test_variants = [(False, False)]
...
def enumerate_sf_layout():
    # Single small case for fast execution
    gran_k_list = (128, ) if get_arch_major() == 9 else (128, )
    for use_ue8m0 in (False, ):
        for with_transpose in (True, ):
            for mn in (512, ):
                for k in (128, ):
                    for num_groups in (1, ):
...
def enumerate_k_grouped_psum_sf_layout():
    for mn, ks_cpu, num_groups, gran_k in enumerate_k_grouped_sf_layout():
        # Single alignment for fast execution
        k_alignment_list = (128, )
